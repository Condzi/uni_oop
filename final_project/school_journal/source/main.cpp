...
int main() {
  ...
  for( auto const& course : s.get_enrolled_courses() ) {
    ...
    sj::debug_print(" %s, %d ects, %s %s", course.get_name().c_str(), course.get_ects(), instructor.get_names().c_str(), instructor.get_surname().c_str() );
  }

  sj::debug_print("\nGrades:");
  for( auto const& grade : s.get_grades() ) {
    auto instructor = db.create_instructor( grade.get_course().get_instructor_id() );
